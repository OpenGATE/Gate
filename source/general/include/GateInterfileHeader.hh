...
void GateInterfileHeader::DoDataRead(std::vector<OutputPixelType> &data) {
  ...
  if (!is) {
      G4cerr << Gateendl <<"Error: the number of pixels that were read from the data file (" << is.gcount() << ") " << Gateendl
	  << "is inferior to the number computed from its header file (" << pixelNumber << ")!" << Gateendl;
