...
#ifndef __GateIntImage_hh__
#define __GateIntImage_hh__
...
class GateIntImage
{
  ...
  bool HasSameResolutionThan(const GateIntImage & image) const;
  bool HasSameResolutionThan(const GateIntImage * pIntImage) const;

  // Access to the image values
  /// Returns the value of the image at voxel of index provided
  inline int GetValue(int index) const { return data[index]; }

  /// Returns a reference on the value of the image at voxel of index provided
  inline int& GetValue(int index)       { return data[index]; }

  /// Returns the value of the image at voxel of coordinates provided
  inline int GetValue(int i, int j, int k) const { return data[i+j*lineSize+k*planeSize]; }

  /// Returns the value of the image at voxel of position provided
  inline int GetValue(const G4ThreeVector& position) const { return data[GetIndexFromPosition(position)]; }

  /// Returns a reference on the value of the image at voxel of position provided
  inline int& GetValue(const G4ThreeVector& position) { return data[GetIndexFromPosition(position)]; }
  /// Sets the value of the voxel of coordinates x,y,z

  inline void SetValue ( int x, int y, int z, int v ) { data[x+y*lineSize+z*planeSize]=v; }
  /// Sets the value of the voxel of index i

  inline void SetValue ( int i, int v ) { data[i]=v; }

  /// Adds a value to the voxel of index provided
  inline void AddValue(int index, int value) { data[index] += value; }
 
  /// Fills the image with a value
  //  inline void Fill(int v) { for (iterator i=begin(); i!=end(); ++i) (*i)=v; }
  inline void Fill(int v) { fill(begin(), end(), v); }
  
  int GetMinValue();
  
  inline int GetOutsideValue()   { return mOutsideValue; } 
  inline void SetOutsideValue( int v ) { mOutsideValue=v; }
  ...
  int GetNeighborValueFromCoordinate(const ESide & side, const G4ThreeVector & coord);
  ...
  /// Writes the image to a file (the format is detected automatically)
  void Write(G4String filename);
