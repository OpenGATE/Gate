...
GateModuleListManager::GateModuleListManager(GateNamedObject* itsMotherObject,
		const G4String& itsName, const G4String& itsElementTypeName,
		G4bool canBeDisabled, G4bool acceptNewElements) :
		GateListManager(itsName, itsElementTypeName, canBeDisabled,
				acceptNewElements), pMotherObject(itsMotherObject) {
}
//--------------------------------------------------------------------------------------------
...
void GateModuleListManager::Describe(size_t indent) {
	...
	G4cout << GateTools::Indent(indent) << "Attached to:        '"
			<< pMotherObject->GetObjectName() << "'\n";
}
//--------------------------------------------------------------------------------------------
