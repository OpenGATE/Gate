...
GateFictitiousFastSimulationModel::GateFictitiousFastSimulationModel(
		...
		G4VFastSimulationModel("Fictitious interaction model"), pTotalCrossSectionsTable(
		NULL), pFictitiousMap( NULL), pTotalDiscreteProcess( NULL), pEnvelopeSolid(
		NULL), m_nApproximations(GatePETVRT::kVolumeTrace), pCurrentFastTrack(
		NULL), pCurrentTrack(NULL), pCurrentTrackStep(NULL), pCurrentFastStep(
		NULL), pPhantomSD(NULL), pInverseTransform(NULL), pTransform(NULL) //, pMaxMaterial(NULL)
{
	...
	m_nNumSecondaries = 0;
	m_nSurfaceTolerance =
			G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() * 3.;
	//Initializing the rest of members
	m_nDiscardEnergy = 0;
	m_nCurrentVelocity = 0;
	m_Initialized2nd = false;
...
G4bool GateFictitiousFastSimulationModel::IsApplicable(
		const G4ParticleDefinition& def) {
	if (G4Gamma::GammaDefinition() == &def)
		return true;
	return false;
}
...
void GateFictitiousFastSimulationModel::DoIt(const G4FastTrack& ft,
		G4FastStep& fs) {
	if (!m_nInitialized) {
		if (!SetParameters(
				GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings())) {
			G4Exception(
					"GateFictitiousFastSimulationModel::DoIt(const G4FastTrack&, G4FastStep&)",
					"Not all necessary things initialized", FatalException,
					"FictitiousMap or TotalDiscreteProcess or TotalCrossSection not set ");
		}
		pFictitiousMap->RegisterCrossSectionsTable(
				GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetTotalDiscreteProcess()->GetTotalCrossSectionsTable(),
				false);
		m_nInitialized = true;
		pTotalCrossSectionsTable =
				GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetTotalDiscreteProcess()->GetTotalCrossSectionsTable();

		m_nDiscardEnergy =
				GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetDiscardEnergy();

		if (m_nDiscardEnergy <= 0) {
			if (GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetFictitiousEnergy()
					<= 0) {
				const GateFictitiousVoxelMap* vmap =
						dynamic_cast<const GateFictitiousVoxelMap*>(GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetFictitiousMap());
				if (vmap) {
					G4ThreeVector vs =
							vmap->GetGeometryVoxelReader()->GetVoxelSize();
					G4double lb =
							pTotalDiscreteProcess->GetTotalCrossSectionsTable()->GetEnergyLimitForGivenMaxCrossSection(
									4.
											/ sqrt(
													vs[0] * vs[0]
															+ vs[1] * vs[1]
					...
					G4cout << "Energy="
							<< GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetFictitiousEnergy()
									/ MeV << " MeV\n";
					G4Exception(
							"GateFictitiousFastSimulationModel::DoIt(const G4FastTrack&, G4FastStep&)",
							"Fictitious energy too small", FatalException,
		...
		if (m_nMinEnergy >= m_nMaxEnergy) {
			G4Exception(
					"GateFictitiousFastSimulationModel::DoIt(const G4FastTrack&, G4FastStep&)",
					"Fictitious eneryg too large", FatalException,
	...
	m_nDistToOut = pEnvelopeSolid->DistanceToOut(m_nCurrentLocalPosition,
			m_nCurrentLocalDirection) + m_nSurfaceTolerance;

//G4cout << "E:" << pCurrentFastTrack->GetPrimaryTrack()->GetDynamicParticle()->GetKineticEnergy() << " \n";
	m_nCurrentInvFictCrossSection =
			1.
					/ pTotalDiscreteProcess->GetTotalCrossSectionsTable()->GetMaxCrossSection(
							m_nCurrentEnergy);
	...
	default:
		G4Exception(
				"GateFictitiousFastSimulationModel::DoIt(const G4FastTrack&, G4FastStep&)",
				"Approximations not known!", FatalException, "This is a bug.");
		break;
	}
...
void GateFictitiousFastSimulationModel::VolumeTrace() {
	...
	do {
		...
		if (m_nPathLength >= m_nDistToOut) // leaves Region before interaction would occur --> no interaction in envelope
				{
			Affine(m_nCurrentLocalPosition, m_nCurrentLocalDirection,
					m_nDistToOut - m_nPathLength + fict);
			...
			pCurrentFastStep->ProposePrimaryTrackFinalPosition(
					m_nCurrentLocalPosition, true);
			pCurrentFastStep->ProposePrimaryTrackFinalMomentumDirection(
					m_nCurrentLocalDirection, true);
			pCurrentFastStep->SetPrimaryTrackFinalKineticEnergy(
					m_nCurrentEnergy);
		...
		assert(
				pTotalCrossSectionsTable->GetCrossSection(currentMaterial,
						m_nCurrentEnergy) * m_nCurrentInvFictCrossSection
						<= 1.);
	...
	G4ThreeVector globalPos = pInverseTransform->TransformPoint(
			m_nCurrentLocalPosition);
	G4ThreeVector globalDir = pInverseTransform->TransformAxis(
			m_nCurrentLocalDirection);
	...
	postStepPoint->SetProcessDefinedStep(
			pTotalDiscreteProcess->SampleActiveProcess(currentMaterial,
					m_nCurrentEnergy));

	// change particle state and calculate secondaries if necessary using G4 methods
	G4VParticleChange* change = pTotalDiscreteProcess->PostStepDoIt(
			*pCurrentTrack, *pCurrentTrackStep);
...
//		postStepPoint->SetProcessDefinedStep ( pTotalDiscreteProcess->GetActiveProcess() );
	...
	G4ParticleChangeForGamma* change4gamma =
			dynamic_cast<G4ParticleChangeForGamma*>(change);
	...
	if (change4gamma) // G4VEmProcess
	{
		...
		if (m_nCurrentEnergy <= m_nMinEnergy) // energy too small --> standard tracking
			...
			pCurrentFastStep->ProposePrimaryTrackFinalPosition(
					m_nCurrentLocalPosition, true);
			pCurrentFastStep->ProposePrimaryTrackFinalMomentumDirection(
					change4gamma->GetProposedMomentumDirection(), false);
			pCurrentFastStep->SetPrimaryTrackFinalKineticEnergy(
					m_nCurrentEnergy);
			pCurrentFastStep->ProposePrimaryTrackFinalPolarization(
					change4gamma->GetProposedPolarization(), false);
		...
		m_nCurrentLocalDirection = pTransform->TransformAxis(
				change4gamma->GetProposedMomentumDirection());
	} else // LowEnergy
	{
		G4ParticleChange* change4lowEnergy =
				dynamic_cast<G4ParticleChange*>(change);
		if (change4lowEnergy) {
			...
			if (m_nCurrentEnergy <= m_nMinEnergy) // energy too small --> standard tracking
				...
				pCurrentFastStep->ProposePrimaryTrackFinalPosition(
						m_nCurrentLocalPosition, true);
				pCurrentFastStep->ProposePrimaryTrackFinalMomentumDirection(
						*change4lowEnergy->GetMomentumDirection(), false);
				pCurrentFastStep->SetPrimaryTrackFinalKineticEnergy(
						m_nCurrentEnergy);
				pCurrentFastStep->ProposePrimaryTrackFinalPolarization(
						*change4lowEnergy->GetPolarization(), false);
				pCurrentFastStep->SetPrimaryTrackPathLength(m_nTotalPathLength);
				AddSecondaries(change);
				return;
			}
			pCurrentTrack->SetPolarization(
					*change4lowEnergy->GetPolarization());
			// update local momentum direction
			m_nCurrentLocalDirection = pTransform->TransformAxis(
					*change4lowEnergy->GetMomentumDirection());
		} else {
			G4Exception("GateFictitiousFastSimulationModel::VolumeTrace()",
					"ParticleChange type not recognized (wrong process)",
...
void GateFictitiousFastSimulationModel::SetTotalDiscreteProcess(
		GateTotalDiscreteProcess* p) {
	assert(p!=NULL);
	pTotalDiscreteProcess = p;
//	SetCrossSectionsTable ( pTotalDiscreteProcess->GetTotalCrossSectionsTable() );
}

void GateFictitiousFastSimulationModel::SetFictitiousMap(
		GateVFictitiousMap* p) {
	if ((pFictitiousMap != NULL) && (p != pFictitiousMap)) {
		G4cout
				<< "Warning! GateFictitiousFastSimulationModel::SetFictitiousVoxelMap: Fictitious voxel map pointer was already set. Fictitious voxel map pointer will be overwritten!\n";
	}
	pFictitiousMap = p;
...
bool GateFictitiousFastSimulationModel::SetParameters(
		const GatePETVRTSettings* settings) {
	if (m_nInitialized) {
		G4Exception(
				"GateFictitiousFastSimulationModel::SetParameters(const GatePETVRTSettings*)",
				"Already initialized", FatalException, "Do nothing.");
		return true;
	} else {
...
// Retrieves the Fast Simulation Manager or creates one if needed.
			...
			if ((theFastSimulationManager =
					anEnvelope->GetFastSimulationManager()) == 0)
				theFastSimulationManager = new G4FastSimulationManager(
						anEnvelope, true);  //isunique
			// adds this model to the Fast Simulation Manager.
			theFastSimulationManager->AddFastSimulationModel(this);
			assert(
					theFastSimulationManager->GetEnvelope()->GetNumberOfRootVolumes()
							== 1);
