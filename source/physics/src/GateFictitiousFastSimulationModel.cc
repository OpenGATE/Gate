...
void GateFictitiousFastSimulationModel::DoIt ( const G4FastTrack& ft, G4FastStep& fs )
{
	if ( !m_nInitialized )
	{
		...
		if ( m_nDiscardEnergy<=0 )
		{
			if ( GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetFictitiousEnergy() <=0 )
			{
				...
				if ( vmap )
				{
					...
					SetMinEnergy ( lb );
#ifdef G4VERBOSE
					G4cout << "GateFictitiousFastSimulationModel: Choose lower energy border = "<< lb/keV << " keV by using information about involved processes, materials, and voxel size." << Gateendl;
#endif
				}
				else
				{
					G4cout << "Energy=" << GatePETVRTManager::GetInstance()->GetOrCreatePETVRTSettings()->GetFictitiousEnergy() /MeV << " MeV" << Gateendl;
		...
		if ( m_nDiscardEnergy>0 )
		{
			G4cout << "GateFictitiousFastSimulationModel: Use discard energy of "<< m_nDiscardEnergy / keV << " keV" << Gateendl;

		}

		G4cout << "GateFictitiousFastSimulationModel: Use fictitious energy of "<< m_nMinEnergy / keV << " keV" << Gateendl;
#endif
...
//G4cout << "E:" << pCurrentFastTrack->GetPrimaryTrack()->GetDynamicParticle()->GetKineticEnergy() << " " << Gateendl;
...
void GateFictitiousFastSimulationModel::SetFictitiousMap ( GateVFictitiousMap* p )
{
	...
	{
		G4cout << "Warning! GateFictitiousFastSimulationModel::SetFictitiousVoxelMap: Fictitious voxel map pointer was already set. Fictitious voxel map pointer will be overwritten!" << Gateendl;
	}
	pFictitiousMap=p;
