...
GateCrossSectionsTable::GateCrossSectionsTable(G4double minEnergy,
		G4double maxEnergy, G4int physicsVectorBinNumber,
		const G4ParticleDefinition* pdef,
		const vector<G4VDiscreteProcess*>& processes) :
		G4PhysicsTable(), m_oInvDensity(), m_oMaterialVec(), m_oProcessVec(
				processes), m_pMaxCrossSection( NULL) {
	...
	pMaterialTableToProductionCutsTable =
			man->GetMaterialTableToProductionCutsTable();
	m_nVerbose = 3;
...
GateCrossSectionsTable::GateCrossSectionsTable(G4double minEnergy,
		G4double maxEnergy, G4int physicsVectorBinNumber,
		const G4ParticleDefinition* pdef, G4VDiscreteProcess& process) :
		G4PhysicsTable(), m_oInvDensity(), m_oMaterialVec(), m_oProcessVec(1,
				&process), m_pMaxCrossSection( NULL) {
...
GateCrossSectionsTable::GateCrossSectionsTable(std::ifstream& in, bool ascii,
		const vector<G4VDiscreteProcess*>& processes) :
		G4PhysicsTable(), m_oInvDensity(), m_oMaterialVec(), m_oProcessVec(
				processes), m_pMaxCrossSection( NULL) {
	GatePETVRTManager* man = GatePETVRTManager::GetInstance();
...
size_t GateCrossSectionsTable::AddMaterial(const G4MaterialCutsCouple* couple) // returns index for that material
	...
	if (m_pMaxCrossSection != NULL) {
		G4cout
				<< "GateCrossSectionsTable::AddMaterial( const G4Material* mat ) : Added material AFTER building maximal! This will most probably lead to wrong results!\n";
	}

	G4PhysicsLinearVector* a = new G4PhysicsLinearVector(m_nMinEnergy,
			m_nMaxEnergy, m_nPhysicsVectorBinNumber);
	const G4double delta = (m_nMaxEnergy - m_nMinEnergy)
			/ m_nPhysicsVectorBinNumber;
	G4DynamicParticle* partTmp = new G4DynamicParticle(
			const_cast<G4ParticleDefinition*>(pParticleDefinition),
			G4ThreeVector(0., 0., 0.)); // deleted by trackTmp
	...
	for (size_t i = 0; i < a->GetVectorLength(); i++) {
		...
		for (std::vector<G4VDiscreteProcess*>::const_iterator it =
			...
			if (dynamic_cast<G4VEmProcess*>(*it)) {
				...
				G4StepPoint* point =
						const_cast<G4StepPoint*>(trackTmp.GetStep()->GetPreStepPoint());
				point->SetMaterialCutsCouple(couple);
				//G4ForceCondition forc=NotForced;
				G4LivermoreRayleighModel* ray =
						dynamic_cast<G4LivermoreRayleighModel*>(*it);
				if (ray) {
					//c=ray->DumpMeanFreePath ( trackTmp,0., &forc );
					c = ray->ComputeMeanFreePath(pParticleDefinition, energy,
							couple->GetMaterial());
					assert(c > 0);
					b += 1. / c;
				} else {
					G4LivermorePolarizedRayleighModel* polray =
							dynamic_cast<G4LivermorePolarizedRayleighModel*>(*it);
					if (polray)

					{
						//c=polray->DumpMeanFreePath ( trackTmp,0.,&forc );
						c = polray->ComputeMeanFreePath(pParticleDefinition,
								energy, couple->GetMaterial());
						assert(c > 0);
						b += 1. / c;
					} else {
						G4LivermoreComptonModel* lcomp =
								dynamic_cast<G4LivermoreComptonModel*>(*it);
						if (lcomp)

						{
							//c=lcomp->DumpMeanFreePath ( trackTmp,0.,&forc );
							c = lcomp->ComputeMeanFreePath(pParticleDefinition,
									energy, couple->GetMaterial());
							assert(c > 0);
							b += 1. / c;
						} else {
							 ...
							 */
							G4Exception(
									"GateCrossSectionsTable::AddMaterial(const G4MaterialCutsCouple*)",
									G4String(
											"'" + (*it)->GetProcessName()
													+ "' is neither G4VEmProcess (='standard') nor G4LivermoreRayleighModel, G4LivermorePolarizedRayleighModel, G4LivermoreComptonModel. At present other processes cannot be used together with fictitious.").c_str(),
									FatalException,
									"Cannot build fast tables.");
...
size_t GateCrossSectionsTable::SetAndBuildProductionMaterialTable() // returns index for that material
{
	...
	if (size() > 0)
		G4cout
				<< "GateCrossSectionsTable::SetAndBuildProductionMaterialTable (): Delete old materials in table!\n";
...
//	size_t nTotalMaterials=G4Material::GetNumberOfMaterials();
	...
	for (size_t m = 0; m < nMaterials; m++) {
		...
		assert(
				pMaterialTableToProductionCutsTable->P2M(m)
						== static_cast<G4int>(couple->GetMaterial()->GetIndex()));
...
bool GateCrossSectionsTable::CheckInternalProductionMaterialTable() const // should return true if internal table correct
{
	const G4ProductionCutsTable* table =
			G4ProductionCutsTable::GetProductionCutsTable();
	size_t nMaterials = table->GetTableSize();
	if (nMaterials != m_oMaterialVec.size())
		return false;
	size_t nTotalMaterials = G4Material::GetNumberOfMaterials();
	for (size_t m = 0; m < nTotalMaterials; m++) {
		...
		if (n >= 0) // also in production
				{
			if (m_oMaterialVec[n]->GetIndex() != m)
				return false;
		}
	}
	return true;
...
bool GateCrossSectionsTable::BuildMaxCrossSection(
		const vector<G4Material*> & vec) {
	if (vec.size() <= 0) {
		G4Exception("BuildMaxCrossSection(const vector<G4Material*>&)",
				"Vector empty!", FatalException, "No Materials in vector.");
		return false;
	}
	size_t i = 0;
	...
	if (involved_mat_index.size() <= 0) {
		G4Exception("BuildMaxCrossSection(const vector<G4Material*>&)",
				"Materials not found in table", FatalException, "Aborting.");
		return false;
	}
...
//		G4double energy=m_nMinEnergy+delta*i;
		...
		if (b < 0) {
			G4Exception("BuildMaxCrossSection(const vector<G4Material*>&)",
					"Negative maximal cross section!", FatalException,
					"Aborting.");
...
void GateCrossSectionsTable::Store(std::ofstream& out, bool ascii,
		size_t num) const {
	assert(num < length());
	...
	if (ascii)
		...
		out.write(reinterpret_cast<const char*>(&m_oInvDensity[num]),
				sizeof(m_oInvDensity[num]));
}

void GateCrossSectionsTable::Retrieve(std::ifstream& in, bool ascii,
		size_t num) {
	assert(num < length());
	...
	if (ascii)
		...
		in.read(reinterpret_cast<char*>(&m_oInvDensity[num]),
				sizeof(m_oInvDensity[num]));
}
...
void GateCrossSectionsTable::StoreTable(std::ofstream& out, bool ascii) const {
	...
	if (ascii) {
		...
		out.write(
				reinterpret_cast<const char*>(&pParticleDefinition->GetParticleName()),
				PARTICLE_NAME_LENGTH);
		out.write(reinterpret_cast<const char*>(&m_nMinEnergy),
				sizeof(m_nMinEnergy));
		out.write(reinterpret_cast<const char*>(&m_nMaxEnergy),
				sizeof(m_nMaxEnergy));
		out.write(reinterpret_cast<const char*>(&m_nPhysicsVectorBinNumber),
				sizeof(m_nPhysicsVectorBinNumber));
...
void GateCrossSectionsTable::RetrieveTable(std::ifstream& in, bool ascii) {
	...
	if (ascii) {
		...
		if (name != pParticleDefinition->GetParticleName()) {
			G4cout
					<< "Try to Retrieve CrossSectionsTable for non-gamma! Particle panic!\n";
		...
		in.read(reinterpret_cast<char*>(&m_nPhysicsVectorBinNumber),
				sizeof(m_nPhysicsVectorBinNumber));
...
G4double GateCrossSectionsTable::GetEnergyLimitForGivenMaxCrossSection(
		G4double crossSection) const {
	G4int i = 0;
	for (i = m_pMaxCrossSection->GetVectorLength(); i >= 0; --i) {
		if ((*m_pMaxCrossSection)[i] >= crossSection)
			break;
	}
	const G4double delta = (m_nMaxEnergy - m_nMinEnergy)
			/ m_nPhysicsVectorBinNumber;
