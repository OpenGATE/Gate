...
git clone -b v7.1.0 https://github.com/Kitware/VTK.git src
cd bin
cmake ../src -DBUILD_SHARED_LIBS=ON -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/home/vgate/Software/VTK/install -DVTK_RENDERING_BACKEND=OpenGL -DVTK_USE_CXX11_FEATURES=ON -DModule_vtkGUISupportQt=ON -DModule_vtkGUISupportQtOpenGL=ON -DModule_vtkRenderingQt=ON -DModule_vtkViewsQt=ON -DVTK_QT_VERSION=5
make -j4 install
cd ..
rm -rf bin src
...
cd Software/ITK
mkdir src bin install
git clone -b v4.13.3 https://github.com/InsightSoftwareConsortium/ITK.git src
cd bin
ccmake ../src -DBUILD_TESTING=OFF -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=/home/vgate/Software/ITK/install -DModule_ITKVtkGlue=ON -DVTK_DIR=/home/vgate/Software/VTK/install/lib/cmake/vtk-7.1/
make -j4 install
cd ..
rm -rf bin src
...
cd Software/RTK
mkdir src bin
git clone -b v5.2.0 https://github.com/InsightSoftwareConsortium/ITK.git src
cd bin
...
cd bin
ccmake ../src -DITK_DIR=/home/vgate/Software/ITK/install/lib/cmake/ITK-4.13/ -DCMAKE_INSTALL_PREFIX=/home/vgate/Software/vv/install
make install
cd ..
rm -rf src bin
...
cd Software/root
mkdir src bin
git clone -b v6-19-02 https://github.com/root-project/root.git src
cd bin
...
git clone -b v10.7.0 https://github.com/Geant4/geant4.git src
cd bin
ccmake ../src -DGEANT4_INSTALL_DATA=ON -DGEANT4_INSTALL_DATADIR=/home/vgate/Software/Geant4/data -DCMAKE_INSTALL_PREFIX=/home/vgate/Software/Geant4/install -DGEANT4_USE_QT=ON -DGEANT4_USE_OPENGL_X11=ON
make install
cd ..
rm -rf src bin
...
unzip libtorch-cxx11-abi-shared-with-deps-1.7.0%2Bcpu.zip
...
cd Software/Gate
mkdir src bin
git clone -b v9.1 https://github.com/OpenGATE/Gate.git src
cd bin
