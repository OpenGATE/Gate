...
__global__ void kernel_voxelized_source_b2b(StackGamma stackgamma1, StackGamma stackgamma2,
											int nb_act, int3 dim_vol, float E, float size_voxel) {
	unsigned int id = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;

	// shared values between threads
	__shared__ int step_plan [5];
	if (threadIdx.x == 0) {
		step_plan[0] = 10000;
		step_plan[1] = 1000;
		step_plan[2] = 100;
	...
	if (id < stackgamma1.size) {
		...
		int level = 0;
		int maxlevel = 5;
		int nmin = (nb_act / step_plan[0]);
		if (nmin*step_plan[0] != nb_act) {++nmin;}
		int pos;
		
		float rnd = Brent_real(id, stackgamma1.table_x_brent, 0);
		pos = (int)(rnd * (float)nmin);

		// search emission spot
		level = 0;
		while (level < maxlevel) {
			if (tex1Dfetch(tex_act_val, pos) < rnd) {
				while (tex1Dfetch(tex_act_val, pos) < rnd) {
					pos += step_plan[level];
					if (pos >= nb_act) {pos = nb_act-1; break;}
				}
			} else {
				while (tex1Dfetch(tex_act_val, pos) > rnd) {
					pos -= step_plan[level];
				...
				pos += step_plan[level]; // correct undershoot
			}
			++level;
		}
