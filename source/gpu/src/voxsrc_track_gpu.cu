...
void GateGPU_VoxelSource_GeneratePrimaries(const GateGPUIO_Input * input, 
  ...
  DD("WARNING : the voxel spacing is assumed to be isotropic");
  assert(input->phantom_spacing_x == input->phantom_spacing_y);
  assert(input->phantom_spacing_x == input->phantom_spacing_z);
  float size_voxel = input->phantom_spacing_x; // mm // FIXME
