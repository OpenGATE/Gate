...
/*-------------------------------------------------------
     ...
     Description of speedConversion.c:
	...
	Fill in the members value and default_value of the LMF_cch structure with a speed 
...
int speedConversion(int cch_index, int dataType)
{
  ...
  if (strcmp(complex_unit.numerator, "error") == 0) {
    printf(ERROR11, plist_cch[cch_index].field, plist_cch[cch_index].unit);
  ...
  if (strcmp(complex_unit.numerator, "error") == 0) {
    printf(ERROR11, plist_cch[cch_index].field,
  ...
  if (numerator_unit.unit[0] == '\0') {
    printf(ERROR11, plist_cch[cch_index].field, plist_cch[cch_index].data);
    printf(ERROR5, cchFileName);
    return (EXIT_FAILURE);
  }
