...
{ 	 //G4cout<<"PrepareNextEvent "<<this <<std::endl;

    // Reset the offline hits collection for the new event
    //GateDigitizerMgr::GetInstance()->ClearOfflineHitsCollection();
  ...
  G4cout << "PrepareNextEvent: "
         << m_hitBuffer.runID << " "
         << m_hitBuffer.eventID << G4endl;
...
/*void GateHitFileReader::PrepareEndOfEvent()
{
  ...
  while (m_hitVector.size()) {
    GateOutputMgr::GetInstance()->GetHitCollection()->insert(m_hitVector.front());
    //m_hitVector.pop();
  }
