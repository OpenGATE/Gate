...
//The track has come  to an end.
    //Esta en G4TrackingManager
    //It gives  all the secondaries generated by the current track. When the track is a primary track  we will get first generation,
 //G4TrackVector* sscind=fpTrackingManager->GimmeSecondaries();
    //I need to get the secondarues of the track
    //atrack->SetUserInformation();

    //El fpTrackingManager inicializado en G4UserTrackingAction que tiene el pre y posUserTackingAction (G4track ) pero no son las que yo llamp


  // esto asi no creo que uncione que estep estoy cogiendo
    //NO ENTiendo la diferencia El tercero e deja porque output constante ? Preg DS
    //atrack->GetStep()->GetfSecondary();
      //atrack->GetStep()->NewSecondaryVector();
    //Maybe here memmory problems (To be checked)
     const G4TrackVector* secondaries=atrack->GetStep()->GetSecondary();

    // G4String volNameTrackV="";
     //auto logVol = atrack->GetLogicalVolumeAtVertex();
     //if ( logVol ) {
      //    volNameTrackV = logVol->GetName();
     //}
