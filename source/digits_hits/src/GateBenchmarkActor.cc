...
void GateBenchmarkActor::UserSteppingAction(const GateVVolume*, const G4Step* step)
{
	...
	const G4double free_path = step->GetStepLength();
	...
	histoEFreePath->Fill(energy_pre/MeV,free_path/mm,weight);
	histoEDeltaE->Fill(energy_pre/MeV,energy_delta/MeV,weight);
	if (deviation) histoEPrimaryDeviation->Fill(energy_pre/MeV,deviation/MeV,weight);
	...
	while (currentSecondary < step->GetSecondary()->size())
	{
		...
		if (deviation_secondary) histoESecondaryDeviation->Fill(energy_pre/MeV,deviation_secondary/deg,weight);
