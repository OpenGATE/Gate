...
G4double GateInverseSquareBlurringLaw::ComputeResolution(G4double energy) const {
	...
	if(m_resolution < 0. ) {
		G4cerr << 	Gateendl << "[GateInverseSquareBlurringLaw::ComputeResolution]:" << Gateendl
      	   <<   "Sorry, but the resolution (" << GetResolution() << ") is invalid" << Gateendl;
		...
		G4cerr <<   Gateendl << "[GateInverseSquareBlurringLaw::ComputeResolution]:" << Gateendl
			<<   "Sorry, but the energy of reference (" << G4BestUnit(GetEnergyRef(),"Energy") << ") is invalid" << Gateendl;
...
void GateInverseSquareBlurringLaw::DescribeMyself (size_t indent) const {
	G4cout << "Inverse Square law for energy blurring" << Gateendl;
