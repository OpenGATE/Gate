...
{
  GateDebugMessageInc( "Actor", 4, "GateDiffCrossSectionActor() -- begin" << Gateendl);
  ...
  GateDebugMessageDec("Actor", 4, "GateDiffCrossSectionActor( ) -- end" << Gateendl);
}
//-----------------------------------------------------------------------------
...
void GateDiffCrossSectionActor::ReadListEnergy(G4String energylist)
{
  ...
  if( !inEnergyFile ) { // file couldn't be opened
    G4cout << "Error: file could not be opened" << Gateendl;
    exit( 1);
  }
...
{
  GateDebugMessageInc("Actor", 4, "GateDiffCrossSectionActor -- Construct - begin" << Gateendl);
  ...
  // Print information
  GateMessage("Actor", 1,
              "\tDiffCrossSection DiffCrossSectionActor    = '" << GetObjectName() << "'" << Gateendl);


  //  ResetData();
  GateMessageDec("Actor", 4, "GateDiffCrossSectionActor -- Construct - end" << Gateendl);
