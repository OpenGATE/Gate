  ...
  case kRevanC_CSR:
  {
      ...
      if(outputPulseT->size()==2){
          ...
          for(unsigned int i=0; i<CoinPermutations.size(); i++){
              ...
              //Only enters once  because we have only three singles in the coincidence. So the parameters are obtained for the middle interaction
             ...
             // dCosPhiE2=pow(computeComptonScatteringAngleError(tempPulse),2);
