...
void GateAugerDetectorActorMessenger::BuildCommands(G4String base)
  ...
  pGenerateArfCmd = new G4UIcmdWithABool((base + "/generateArf").c_str(), this);
  pGenerateArfCmd->SetGuidance("Generate ARF root file");
  pGenerateArfCmd->SetParameterName("GenerateArf", false);

  pArfFilenameCmd = new G4UIcmdWithAString((base + "/arfFilename").c_str(), this);
  pArfFilenameCmd->SetGuidance("ARF root name");
  pArfFilenameCmd->SetParameterName("ArfFilename", false);

  pProfileNoiseFWHMCmd = new G4UIcmdWithADoubleAndUnit((base + "/setProfileNoiseFWHM").c_str(),
                                                       this);
...
void GateAugerDetectorActorMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue)
  {
  if (cmd == pMinTOFCmd)
    pActor->setMinTOF(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pMaxTOFCmd)
    pActor->setMaxTOF(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pMinEdepCmd)
    pActor->setMinEdep(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pMaxEdepCmd)
    pActor->setMaxEdep(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pProfileDirectionCmd)
    pActor->setProjectionDirection(G4UIcmdWith3Vector::GetNew3VectorValue(newValue));
  if (cmd == pMinProfileCmd)
    pActor->setMinimumProfileAxis(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pMaxProfileCmd)
    pActor->setMaximumProfileAxis(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pSizeProfileCmd)
    pActor->setProfileSize(G4UIcmdWithAnInteger::GetNewIntValue(newValue));
  if (cmd == pProfileNoiseFWHMCmd)
    pActor->setProfileNoiseFWHM(G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue));
  if (cmd == pGenerateArfCmd)
    pActor->setGenerateArf(G4UIcmdWithABool::GetNewBoolValue(newValue));
  if (cmd == pArfFilenameCmd)
    pActor->setArfFilename(newValue);
