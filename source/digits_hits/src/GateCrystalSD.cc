...
G4bool GateCrystalSD::ProcessHits(G4Step*aStep, G4TouchableHistory*)
{
  ...
  //Get information about gamma ( generated by ExtendedVSource, if it is from positronium decay ( or deexcitation) )
  G4int source_kind = 0;
  G4int decay_model = 0;
  G4int gamma_kind = 0;
  ...
  if ( primary_particle != nullptr )
  {
   ...
   if ( info != nullptr )
   {
    source_kind = static_cast<G4int>( info->GetSourceKind() );
    decay_model = static_cast<G4int>( info->GetDecayModel() );
    gamma_kind = static_cast<G4int>( info->GetGammaKind() );
   }
  }  
...
/*  GateSystemComponent* baseComponent = GetSystem()->GetBaseComponent();*/
 ...
 // Seb Modif 5/4/2016 
  ...
  aHit->SetSourceKind( source_kind );
  aHit->SetDecayModel( decay_model );
  aHit->SetGammaKind( gamma_kind );
