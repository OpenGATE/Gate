...
: GateVVolume(itsName, itsFlagAcceptChildren, depth),
  m_trd_solid(0), m_box_solid(0), m_trpd_solid(0), m_trpd_log(0), m_cavity(1),
  m_Messenger(0)
{
...
GateTrpd::GateTrpd(const G4String& itsName,
                   ...
                   G4int depth)
  : GateVVolume(itsName, itsFlagAcceptChildren, depth),
    m_trd_solid(0), m_box_solid(0), m_trpd_solid(0), m_trpd_log(0), m_trd_log(0), m_cavity(0),
    m_Messenger(0)
{
...
G4LogicalVolume* GateTrpd::ConstructOwnSolidAndLogicalVolume(G4Material* mater, G4bool flagUpdateOnly)
{
  ...
  if (!flagUpdateOnly || !m_trpd_solid) {
    ...
    delete m_trd_log;
    m_trd_log
      = new G4LogicalVolume(m_trd_solid, mater, GetLogicalVolumeName(),0,0,0);
...
void GateTrpd::DescribeMyself(size_t indent)
{
  ...
  G4cout << GateTools::Indent(indent) << "Is cavity inside the trapezoid: " << isTrpdCavity() << "\n";
