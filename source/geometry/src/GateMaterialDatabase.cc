...
void GateMaterialDatabase::AddMDBFile(const G4String& filename) {
	...
	GateMessage("Materials", 1,
			"New material database added - Number of files: "<<mMDBFile.size() << Gateendl);
}
//-----------------------------------------------------------------------------
...
G4Element* GateMaterialDatabase::GetElement(const G4String& elementName) {
	GateMessage("Materials", 5,
			"GateMaterialDatabase::GetElement("<<elementName<<")\n");
	...
	if (!element) {
		...
		if (!element)
			GateError(
					"GateMaterialDatabase: failed to read the element '" << elementName << "' in the database file!");
	}
	return element;
}
//-----------------------------------------------------------------------------
...
G4Material* GateMaterialDatabase::GetMaterial(const G4String& materialName) {
	GateMessage("Materials", 3,
			"GateMaterialDatabase::GetMaterial("<<materialName<<")\n");
	...
	if (!material) {
		...
		if (!material)
			GateError(
					"GateMaterialDatabase: failed to read the material '" << materialName << "' in the database file!");
	}
	return material;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
G4Element* GateMaterialDatabase::ReadElementFromDBFile(
		const G4String& elementName) {
	GateMessage("Materials", 5,
			"GateMaterialDatabase::ReadElementFromDBFile("<<elementName<<")\n");
	...
	if (!Creator)
		GateError(
				"GateMaterialDatabase: could not find the definition for element '" <<elementName << "' in material files");
	if (nDef > 1)
		GateWarning(
				"GateMaterialDatabase: Multiple definition of element: "<<elementName<<".\nThe definition in "<<fileName<<" was used.\n");
...
//-----------------------------------------------------------------------------
G4Material* GateMaterialDatabase::ReadMaterialFromDBFile(
		const G4String& materialName) {
	GateMessage("Materials", 3,
			"GateMaterialDatabase::ReadMaterialFromDBFile("<<materialName<<")\n");

	GateMaterialCreator *Creator = 0;
	GateMaterialCreator *CreatorTemp = 0;
	G4Material* material = G4NistManager::Instance()->FindOrBuildMaterial(
			materialName);
	int nDef = 0;
	G4String fileName = "";
	...
	if (!Creator) {
		GateError(
				"GateMaterialDatabase: could not find the definition for material '" << materialName << "' in material files");
		GateError(
				"Check if you have defined a material database file in the macro file.");
	}

	if (nDef > 1)
		GateWarning(
				"GateMaterialDatabase: Multiple definition of material: "<<materialName<<".\nThe definition in "<<fileName<<" was used.\n");
	...
	if ((materialName == "Water") && (water_MPT == NULL)) {
		...
		G4double pp_water[num_water] = { 2.034E-9 * GeV, 4.136E-9 * GeV };
		G4double rindex_water[num_water] = { 1., 1. };
		G4double abs_water[num_water] = { 1000. * cm, 1000. * cm };
