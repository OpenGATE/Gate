...
GateVSystem::GateVSystem(const G4String& itsName, G4bool isWithGantry) :
		GateClockDependent(itsName, false), m_BaseComponent(0), m_mainComponentDepth(
				isWithGantry ? 1 : 0) {
	// Next lines were added for the multi-system approach
	G4String itsOwnName =
			GateSystemListManager::GetInstance()->GetInsertedSystemsNames()->back();

	// Papa's Debugging : number of system for headID in case of multi-system
	G4int sysNumber =
			GateSystemListManager::GetInstance()->GetInsertedSystemsNames()->size();
...
GateSystemComponent* GateVSystem::FindComponent(const G4String& componentName,
		G4bool silent) const {
	// Ask the component tree to look for the component
	GateSystemComponent* aComponent = m_BaseComponent->FindSystemComponent(
			GetObjectName() + "/" + componentName);
...
GateArrayComponent* GateVSystem::FindArrayComponent(
...
GateBoxComponent* GateVSystem::FindBoxCreatorComponent(
		const G4String& componentName) const {
...
GateCylinderComponent* GateVSystem::FindCylinderCreatorComponent(
		const G4String& componentName) const {
...
//-----------------------------------------------------------------------------
// Finds a cylindercreator-component from its name
GateWedgeComponent* GateVSystem::FindWedgeCreatorComponent(
		const G4String& componentName) const {
...
GateOutputVolumeID GateVSystem::ComputeOutputVolumeID(
	...
	if (nVerboseLevel)
		G4cout << "[" << GetObjectName() << "::ComputeOutputVolumeID]:\n"
				<< "\tComputing the output volume ID for the hit in volume:"
				<< aVolumeID << Gateendl;

		// Ask the component-tree to compute the output-volume ID
	outputVolumeID[0] = ComputeSubtreeID(m_BaseComponent, aVolumeID,
			outputVolumeID, 0);
...
G4int GateVSystem::ComputeSubtreeID(GateSystemComponent* aComponent,
	...
	for (GateSystemComponent::child_iterator ichild = aComponent->begin();
		...
		subtreeResult = ComputeSubtreeID((GateSystemComponent*) (*ichild),
				volumeID, outputVolumeID, depth + 1);
		if (subtreeResult >= 0) {
...
G4int GateVSystem::ComputeComponentID(GateSystemComponent* aComponent,
...
G4int GateVSystem::ComputeMainComponentID(GateSystemComponent* aComponent,
		const GateVolumeID& volumeID) {
	G4int copyNo = -1;
	G4bool found = false;
	...
	GateObjectRepeaterList* repeaterlist =
			aComponent->GetCreator()->GetRepeaterList();
...
//-----------------------------------------------------------------------------
GateSystemComponentList* GateVSystem::MakeComponentListAtLevel(
		G4int level) const {
	G4String LevelName = "level_" + InttoG4String(level);
	GateSystemComponentList* currentList = new GateSystemComponentList(
...
//-----------------------------------------------------------------------------
size_t GateVSystem::ComputeNofSubCrystalsAtLevel(size_t level,
	...
	if (level == treeDepth - 1) {
		...
		size_t n =
				enableList[level + 1] ?
						ComputeNofElementsAtLevel(level + 1) : 1;
...
//-----------------------------------------------------------------------------
size_t GateVSystem::ComputeIdFromVolID(const GateOutputVolumeID& volID,
		std::vector<G4bool>& enableList) const {
...
//-----------------------------------------------------------------------------
GateVolumeID* GateVSystem::MakeVolumeID(
		const std::vector<G4int>& numList) const {
	...
	ans->push_back(
			GateVolumeSelector(
					GateDetectorConstruction::GetGateDetectorConstruction()->GetWorldVolume()));
	if (vol)
		ans->push_back(GateVolumeSelector(vol));
	else
		return ans;
	...
	for (std::vector<G4int>::const_iterator it = numList.begin();
		...
		if (num >= 0) {
			size_t numChild = 0;
			while (num
					>= (G4int) comp->GetChildComponent(numChild)->GetVolumeNumber()) {
			...
			while (last_vol) {
				...
				if (!logical->IsDaughter(vol)) {
					...
					if (pb)
						return ans; // no last_vol child is ancestor of vol...
				} else {
					ans->push_back(vol);
					last_vol = vol;
					break;
...
//-----------------------------------------------------------------------------
G4ThreeVector GateVSystem::ComputeObjectCenter(
		const GateVolumeID* volID) const {
	...
	return ok ?
			G4ThreeVector((x[0] + x[1]) / 2, (y[0] + y[1]) / 2,
					(z[0] + z[1]) / 2) :
			G4ThreeVector(0, 0, 0);
}
//-----------------------------------------------------------------------------
