...
public:
	...
	G4String GetImageFilename() const {
	...
	G4ThreeVector GetIsoCenter() const {
		return mIsoCenter;
	}
	//-----------------------------------------------------------------------------
	...
	inline G4ThreeVector GetHalfSize() const {
		return mHalfSize;
	}
	/// Gets the Image
	...
	inline const ImageType* GetImage() const {
		return pImage;
	}

	/// Returns the volume's transform matrix
	inline G4RotationMatrix GetTransformMatrix() const {
	...
	G4String GetMaterialNameFromLabel(LabelType l) {
		...
		if (mi == mLabelToMaterialName.end()) {
			G4cerr << "GateVImageVolume<" << GetObjectName()
					<< ">::GetMaterialNameFromLabel : Could not find material of label "
					<< l << " in correspondence table\n";
			exit(0);
		}
		return (*mi).second;
	}
	...
	virtual void GetPhysVolForAVoxel(const G4int, const G4VTouchable &,
			G4VPhysicalVolume **, G4NavigationHistory &) const {
	}
	//-----------------------------------------------------------------------------
