...
GateSplitManager::GateSplitManager(G4int nAliases, G4String* aliases,
		G4String platform, G4String pbsscript, G4String condorscript,
		G4String macfile, G4int nSplits, G4int time) {
	toPlatform = new GateToPlatform(nSplits, platform, pbsscript, condorscript,
			macfile, time);
...
void GateSplitManager::CheckEnvironment() {
	...
	if (err == 2)
		cout << "Environment variable GC_GATE_EXE_DIR not set !" << endl;
	if (err == 3)
		cout << "Environment variable PWD not set !" << endl;
	if (err > 0)
		CleanAbort();
...
void GateSplitManager::StartSplitting() {
	...
	if (!dirstream) {
		...
		if (m_verboseLevel > 1)
			cout << "Information : Creating a .Gate directory... ";
	...
	if (err)
		...
		outputMacDir = macParser->GetOutputMacDir();
...
void GateSplitManager::CleanAbort() {
	ofstream tmp1, tmp2;
	if (macParser)
		macParser->CleanAbort(tmp1, tmp2);
