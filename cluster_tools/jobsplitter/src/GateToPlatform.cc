...
GateToPlatform::GateToPlatform(G4int numberOfSplits, G4String thePlatform,
		G4String thePbsscript, G4String theCondorScript, G4String outputMacName,
		G4int time) {
...
int GateToPlatform::GenerateOpenPBSScriptfile() {
	...
	for (G4int i = 1; i <= nSplits; i++) {
		...
		if (!inFile) {
			cout << "Error : could not access openPBS script template file! "
					<< pbsScript << endl;
			return (1);
		}
		...
		if (!scriptFile) {
			cout << "Error : could not create script file! " << out_name
					<< endl;
			return (1);
		}
			...
			if (buf.find("#") != 0 || buf.find("#PBS") == 0) {
				...
				if (pos < buf.length())
					buf = buf.substr(0, pos)
							+ outputDir.substr(0, outputDir.rfind("/"))
							+ buf.substr(pos + 10);
				pos = buf.find("GC_LOG");
				if (pos < buf.length())
					buf = buf.substr(0, pos) + "log" + cnt.str()
							+ buf.substr(pos + 6);
				pos = buf.find("GC_ERR");
				if (pos < buf.length())
					buf = buf.substr(0, pos) + "err" + cnt.str()
							+ buf.substr(pos + 6);
				pos = buf.find("GC_JOBNAME");
				if (pos < buf.length()) {
					//openPBS max_jobname_length=15
					char jobname[16] = "";
					strncpy(jobname, outputMacfilename.c_str(),
							15 - cnt.str().length());
					buf = buf.substr(0, pos) + jobname + cnt.str()
							+ buf.substr(pos + 10);
				}
				...
				if (useTiming == 1)
					timestr = "time ";
				if (pos < buf.length())
					buf = timestr + dir + "Gate " + outputDir + cnt.str()
							+ ".mac" + buf.substr(pos + 7);
			}
			scriptFile << buf << endl;
		}
		scriptFile.close();
...
int GateToPlatform::GenerateOpenMosixSubmitfile() {
	...
	for (G4int i = 1; i <= nSplits; i++) {
		if (useTiming == 1)
			submitFile << "\\time " << dir + "Gate " << outputDir << i
					<< +".mac" << " 2>timefile" << i << " &" << endl;
		else
			submitFile << dir + "Gate " << outputDir << i << +".mac" << " &"
					<< endl;
...
int GateToPlatform::GenerateCondorSubmitfile() {
	...
	if (!scriptFile) {
		cout << "Error : could not open the condor script file! " << scriptFile
				<< endl;
		return (1);
	}
	while (!scriptFile.eof()) {
		...
		scriptline = buffer;
		if (scriptline.contains("#GJS PART => DO NOT REMOVE") != 0)
			noCopy = 1;
		if (noCopy == 0) {
			if (scriptline.contains("Executable") != 0
					&& scriptline.contains("$GC_EXEC") != 0)
				submitFile << "Executable     = " << dir + "Gate" << endl;
			else
				submitFile << scriptline << endl;
...
int GateToPlatform::GenerateXgridSubmitfile() {
	...
	if (!submitXgridFile) {
		cout << "Error : could not create submit file! " << submitXgridFile
				<< endl;
		return (1);
	}
	submitXgridFile << "{" << endl;
	submitXgridFile << "jobSpecification = {" << endl;
	submitXgridFile << "applicationIdentifier = \"com.apple.xgrid.cli\";"
			<< endl;
	...
	for (int i = 0; i < nSplits; i++) {
		submitXgridFile << i << +" = {arguments =(" << outputDir << i
				<< +".mac); command =\"" << dir << +"Gate\"; }; " << endl;
	}
	submitXgridFile << "};" << endl;
	submitXgridFile << "};" << endl;
	submitXgridFile << "}" << endl;
